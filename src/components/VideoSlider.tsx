...
const VideoSlider = ({ videoUrl, onFrameCapture, className = "", initialTime = 0 }: VideoSliderProps) => {
	...
	const handleVideoError = (e: any) => {
		const video = videoRef.current;
		const nativeError = (video as any)?.error || (e?.target && (e.target as any).error);
		...
		const msg = nativeError?.message || e?.message || '';
	...
	const captureCurrentFrame = () => {
		if (videoRef.current && canvasRef.current && onFrameCapture) {
			...
			try {
				...
				// Likely a CORS-tainted canvas when video is cross-origin without proper headers
				console.warn('Frame capture failed (likely CORS):', err);
			}
		}
	};
			...
			<div className={`bg-destructive/10 border border-destructive/20 rounded-lg p-4 ${className}`}>
				...
				<div className="mt-2">
					...
					<video
						...
						playsInline
						muted
						// Do not set crossOrigin on fallback to avoid stricter CORS
					>
			...
			<div className={`space-y-3 ${className}`}>
				...
				<div className="bg-gray-50 rounded-lg p-3">
					...
					<video
						...
						playsInline
						muted
						// No crossOrigin here either to reduce CORS strictness on plain playback
					>
						Your browser does not support video playback.
					</video>
				</div>
